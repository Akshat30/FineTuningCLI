...
3. Ensure that Python3 is installed (`python3 --version`)
4. Run `pip install openai` 
5. Run `pip install python-dotenv` 
6. Create a file called .env in the same directory, type `API_KEY=` and enter your OpenAI API Key next to `API_KEY=`. do not include quotations. See below for an example.
7. Run `python3 cli.py`

### The program should now be running! Check out the Features section at the bottom to see what you can do from here!
